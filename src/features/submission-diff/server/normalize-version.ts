...
export async function normalizeSubmissionFile(
	...
	await Promise.all(
		[...bundle.figures].map(([sha, figureBytes]) =>
			linkFigure(sha, figureBytes),
		),
	);
	// Per-style CSS is trusted-by-construction (generated by the sidecar from
	// validated styles.xml values), so it is NOT run through the HTML sanitizer.
	const [htmlKey, cssKey] = await Promise.all([
		persistHtml(trustedHtml),
		persistCss(bundle.css),
	]);
