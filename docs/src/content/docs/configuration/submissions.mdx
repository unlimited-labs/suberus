...
submission, regardless of type: how long titles and abstracts may be, which files
are accepted, the **guidelines** authors and reviewers read, and whether Suberus
...
4. Under *Files*, set the **max file size** and tick the **allowed file types**.

5. Edit the **guidelines** and extraction options, then click **Save All Settings**.
...
| Field | What it does | What to enter / Notes |
|---|---|---|
| **Max file size (MB)** | Upload size limit for submission files. | 1–100 MB. |
| **Allowed file types** | Which file formats authors may upload. | Tick any of **PDF, DOCX**. |
...
| **Structure-based extraction** | Reads metadata from the document's structure/formatting. Instant, no extra infrastructure. | A status dot shows whether the *Docling* service is healthy. |
