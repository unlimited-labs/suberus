...
- **Untimed (poster / lightning) sessions are skipped** — they never count as
  scaffold and their contents are never overwritten. **Sessions holding an
  [invited talk](/planner/manual-scheduling/#invited-talks) are skipped** too, since the
  autoplanner rewrites the sessions it fills.
...
| "A session in this proposal now holds an invited talk" | An [invited talk](/planner/manual-scheduling/#invited-talks) was added after the proposal was generated | Re-run the autoplanner. Sessions holding an invited talk are left out of the plan, so it will skip that one. |
