...
- **Conference Timezone** is the zone **all** session times and **days** are shown
  in — in the planner and in the public program alike (times are stored in UTC).
  It does not follow the browser's zone, so an organiser or attendee abroad sees
  the venue's clock, and the public program states the zone under its day selector.
