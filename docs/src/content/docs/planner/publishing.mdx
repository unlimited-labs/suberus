...
4. Flip the **Make the program public** switch — going public is visible to
...
| **Published** | Everyone, at `/program`. | Flip the switch, then **Publish**. |
...
- A **timezone note** under the day selector — *All times shown in
  Europe/Warsaw (GMT+2)* — because days and times are rendered in the
  **[Conference Timezone](/settings/conference/)**, never in the visitor's own zone.
  An attendee reading the program from Tokyo sees the same days and clock times as
  one reading it in the venue. The note is hidden if no Conference Timezone is set.
