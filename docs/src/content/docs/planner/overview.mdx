...
1. **[Set up](/planner/setup/)** the basics: conference dates & timezone, planner
   visible hours, rooms, and (optionally) program tracks.

2. Build the schedule — **[by hand](/planner/manual-scheduling/)**, with the
   **[autoplanner](/planner/autoplanner/)**, or both.

3. Fix anything the **Issues panel** flags (clashes, missing rooms, sessions
   outside the conference dates).

4. **[Publish](/planner/publishing/)** to make the program public.
...
- **Program track** — an optional colour tag for grouping related sessions. Add a
  ...
  These are separate from the subject **[Tracks](/configuration/tracks/)** authors
  pick at submission.
