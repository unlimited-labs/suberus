...
| **Header controls** | Day/Week view toggle, previous/next/**Today**, a **Conference start** jump, the **Rooms** filter (show/hide room columns), **Auto-plan**, **+ New**, **Clear plan**, and **Publish**. |
...
- **Untimed session** — a poster or lightning block: it has its own start and end,
  but the presentations inside share the whole window instead of getting
  individual times, and there is no capacity limit.
- **Break** — a non-talk block (coffee, lunch). A break with **no room** spans
  **all rooms**.
- **Invited talk** — a keynote, invited lecture or sponsor address that never went
  through submission. Added straight into a session from the session editor; see
  **[Manual scheduling](/planner/manual-scheduling/#invited-talks)**.
...
<CardGrid>
	...
	<LinkCard title="AI assistant" href="/managing/ai-assistant/" description="Let a connected assistant read and build the program for you." />
</CardGrid>
