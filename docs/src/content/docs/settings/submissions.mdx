...
Whatever the submission type, a few rules apply to all of them, and this tab is
where you set them: how long titles and abstracts may run, the guidelines authors
and reviewers read, and whether Suberus automatically extracts metadata from
uploaded documents.

<Aside type="note" title="Where to find it">
**Settings › Submissions.** Several sections, but a single **Save All
...
**[Submission Types](/settings/submission-types/)**. This tab is for rules
...
<Aside type="caution" title="Out-of-range values block the save">
If a value falls outside its range, or a **Min** exceeds its **Max**, the
message appears under the field and **Save All Settings** does nothing until
you correct it — the guidelines and extraction options on this tab are not
saved either, since the whole tab saves as one.
</Aside>
...
| Field | What it does | What to enter / Notes |
|---|---|---|
| **Submission Guidelines** | Guidance authors see while submitting. | Markdown, syntax-highlighted as you type. Click a **placeholder badge** (e.g. `{{maxTitleLength}}`) to insert a dynamic value that always reflects the current limits. |
...
| Field | What it does | What to enter / Notes |
|---|---|---|
| **Review Guidelines** | Guidance reviewers see in the review form sidebar. | Markdown, syntax-highlighted as you type. e.g. how to give constructive feedback. |
...
<CardGrid>
	<LinkCard title="Submission Types" href="/settings/submission-types/" description="Per-type reviewers, blind mode, file format." />
