...
| **Upload your own .p12** | Use an organizational or qualified certificate for full trust. | A `.p12`/`.pfx` file under 1 MB. Leave **Password** empty if the file is not password-protected. Any other file type is refused before upload. |
...
<Aside type="danger" title="Signing on, but no certificate stored">
If the certificate card shows **“Signing is on but no certificate is stored”**,
document generation fails until you upload or regenerate one. Older conferences
kept the certificate outside the settings; those installations land in this state
until the certificate is re-uploaded. Generation stops rather than quietly handing
out unsigned documents, so fix it before participants request theirs.
</Aside>
