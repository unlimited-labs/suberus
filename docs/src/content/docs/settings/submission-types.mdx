...
**Settings › Submission Types.** Each type is a collapsible panel; click one
...
1. Open **Settings › Submission Types** and click the type you want to set up.
...
3. Choose the **Content Format** — *Text* for typed abstracts, *File Upload* for
   documents. For file uploads, choose the allowed extension and set the
   **max file size**.
...
6. Optionally enable **scoring**, **confidence level**, **review attachments**, and
...
| **Max submissions per user** | Caps how many submissions of this type a single author may have. Counts only submissions the user **owns** — being a **co-author** on someone else's submission never counts. Drafts, withdrawn and rejected submissions don't count (withdrawing or a rejection frees a slot). | 0–1000. **0 = unlimited** (default). Not shown for Exhibitor. |
...
| **Allowed file extension** | *(File Upload only)* The single document type accepted for this submission type: **pdf** or **docx**. | One must be selected, or the type won't save. Full Paper defaults to **docx**. |
...
| **Enable acknowledgment** | Adds an optional **Acknowledgment** box to the submission form, where authors credit funding, grants and people. When a DOCX is uploaded and extraction is on, an *Acknowledgments* section found in the document fills the box automatically. | Optional, never required. The text is shown on the submission page, in the public programme and in the CSV export — it is **not** shown to reviewers. |
...
<Aside type="caution" title="What blocks a save">
Three combinations are refused, with the reason shown next to **Save**: a
**File Upload** format with no allowed extension, **scoring** enabled with no
criteria, and **Required reviewers** below 1 (only *Exhibitor*, which is never
reviewed, may sit at 0).
</Aside>
