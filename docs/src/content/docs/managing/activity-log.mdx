...
| **Settings events** | Changes to the [Conference tab](/settings/conference/), listing exactly which fields changed. Recorded whether the change came from the settings screen or from an [AI assistant](/managing/ai-assistant/). |
| **MCP client events** | An outside application registering itself against the AI assistant connection (with its client ID, declared name and redirect URLs), and any later change to that declared metadata. |
...
MCP client registrations have no performer — nobody in Suberus triggers them,
the outside application does — so they are shown as *system*, with the
application's name as the subject.
...
<CardGrid>
	...
	<LinkCard title="AI assistant (MCP)" href="/managing/ai-assistant/" description="Which applications connected, and what they may do." />
</CardGrid>
