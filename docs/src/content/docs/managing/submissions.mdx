...
**Side-by-side / Inline** toggle drives all of them: side-by-side reconstructs
the old and new value in two columns, inline shows one unified redline.
...
| **Comparing v_A → v_B** | A sticky header naming the two versions and their dates, with a **File changed / File unchanged** badge. If two different versions are completely identical, a banner says so. |
| **Title** | The title text, with any change highlighted. |
| **Authors** | The author composition (name, e-mail, affiliation, and who presents), with a changed **author line-up** highlighted. Shown to editors only. |
| **Keywords** | The keyword/tag set, with any change highlighted. Shown to editors only. |
| **Attached file** | The file name on each side, with a **File changed / File unchanged** indication. |
...
| **File contents** | Shown when the attached file changed. **Side-by-side** reconstructs each version's document in its own panel (with **synced scrolling**, so both sides stay aligned); **Inline** shows a single unified redline of the document. Changed **figures** are framed and labelled (removed / added); equations are rendered. The document redline is prepared in the background once a revision is uploaded, so it may take a moment to appear the first time you open the comparison. |
