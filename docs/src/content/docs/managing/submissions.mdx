...
**audit history**. The **Details** panel also shows the **submission number**
(e.g. `12` — the same number as the list's **No.** column, and the one used to match
camera-ready files) and the **submitter** — the user who created the submission — as a
link to their profile. The action buttons available
depend on the current status.

| Action | When available | What it does |
|---|---|---|
| **Submit draft** | `DRAFT` — **Administrators and Editors** (not for **Exhibitor** or **Invited** entries) | Sends a draft the author never finished into review, on their behalf. The presenting author gets the usual submission-received e-mail. Cannot be undone. The per-type submission limit is not enforced. |
...
| **Edit submission** | Any — **Administrators only** (not **Exhibitor** presentations — edit those via the **[Exhibitors](/managing/exhibitors/)** flow) | Opens the submission form to fix title, abstract/content, authors, keywords, track, or the uploaded file. Saves **in place on the current version — no new version is created** and the status is unchanged. For a `DRAFT` the form offers two buttons: **Save Draft** (keeps it a draft) and **Submit** (saves, then sends it into review exactly as **Submit draft** does). Logged in the activity history as *Edited by admin*. Editors do not see this action. A connected [AI assistant](/managing/ai-assistant/) can make the same edit, and can fetch the uploaded file to check what it says. |
| **Change submitter** | Any — **Administrators and Editors** (not **Exhibitor** or **Invited** entries) | Hands the submission over to another participant account. The submitter owns the record: it appears among *their* submissions, they upload revisions to it and they receive its reminders. This is separate from the **author list** — correcting who is credited on the abstract is an **Edit submission** job. Only active accounts can be picked. Logged in the activity history as *Submitter changed*, with both names. A connected [AI assistant](/managing/ai-assistant/) can do the same. |
| **Delete** | Any | Permanently removes the submission. Use with care. |

🖼️ [Screenshot placeholder: the **Change submitter** dialog — current submitter, search box, and the list of accounts that can take the submission over]
