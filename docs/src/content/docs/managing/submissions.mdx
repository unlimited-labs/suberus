...
| **Comparing v_A → v_B** | A sticky header naming the two versions and their dates, the inserted/deleted character counts, and a **File changed / File unchanged** badge. |
| **Title** | Inline redline of the title. Insertions are underlined, deletions struck through (colour is never the only cue). |
| **Authors** | Inline redline of the author composition (name, e-mail, affiliation, and who presents) between the two versions — so a changed **author line-up** is visible at a glance. Shown to editors only. |
| **Keywords** | Inline redline of the keyword/tag set between the two versions. Shown to editors only. |
| **Attached file** | The file name on each side. (An inline redline of the file *contents* is planned for a later release.) |
| **Author's note for v_B** | The comment the author left when submitting that version, if any. |
| **Content** | Side-by-side (or inline) redline of the abstract/content text. |

🖼️ [Screenshot placeholder: side-by-side version comparison page]
