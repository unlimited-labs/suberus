...
| **File contents** | Shown when the attached file changed. **Side-by-side** reconstructs each version's document in its own panel (with **synced scrolling**, so both sides stay aligned); **Inline** shows a single unified redline of the document. Changed **figures** are framed and labelled (removed / added); equations are rendered. |
