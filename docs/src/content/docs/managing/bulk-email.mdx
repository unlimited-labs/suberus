...
3. Click **Send campaign** and confirm in the dialog that appears — it shows the recipient
   count and warns that the send cannot be stopped once started. After confirming, the
   campaign is queued and delivered one recipient at a time, with a progress bar showing
   how many have been processed. You can close the browser; delivery continues on the server.
...
| **Body** | The message source, in the chosen format. Syntax-highlighted for Markdown and MJML. Supports placeholders. |
...
| **Send campaign** | Asks for confirmation (the send cannot be stopped once started), then renders the final body, queues the campaign, and starts delivery. |
