...
If you cancel the **last** active reviewer and none remain, the submission drops
back out of `UNDER_REVIEW` — to `SUBMITTED` (first round) or `RESUBMITTED` (later
rounds) — so the **desk accept / desk reject** and **assign reviewer** actions
