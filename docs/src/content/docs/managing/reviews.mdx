...
the review screen shows a **Changes since previous version** panel — an inline
redline of the title and content against the previous version, so the reviewer can
see exactly what the author revised. For a fuller view, the **Compare versions**
button opens a dedicated comparison page with side-by-side panels and pickers to
compare any pair of versions (toggle between side-by-side and inline). Author names
stay hidden under single/double-blind exactly as elsewhere; only the title and
content are diffed.

🖼️ [Screenshot placeholder: reviewer Compare versions page — side-by-side panels]
</Aside>
...
<CardGrid>
	...
	<LinkCard title="Reminders" href="/configuration/reminders/" description="Automatic nudges before review deadlines." />
	<LinkCard title="Submission Types" href="/configuration/submission-types/" description="Reviewers required, deadlines, and blind mode per type." />
</CardGrid>
