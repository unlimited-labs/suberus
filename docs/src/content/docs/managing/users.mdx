...
| **Fee** | Whether their conference fee is marked paid. Click the badge to open the fee dialog right here — pick a fee type and **Save**, or **Unmark as Paid** to reverse it, without opening the user. |
...
Use **Add User** (top right) to create an account yourself — see
[below](#add-a-user-yourself). Search by name, toggle columns, and use **Export XLSX** to download the full list
...
the user's submission-role badges; tap the name block to open the user, or tap
the fee badge to change the fee status without leaving the list. Search is
...
**Add User** (top right of the Users list, available to Admins and Editors) creates
a participant account on the spot — useful for someone who registered by e-mail,
phone, or at the desk. The account is **active immediately**: no invitation, no
waiting for the person to sign up.

🖼️ [Screenshot placeholder: Add User dialog]

| Field | Notes |
|---|---|
| **First name**, **Last name** | Required. |
| **Title** | Academic title (Dr, Prof, …). Optional. |
| **Affiliation** | Institution/organisation. A new name is added to the affiliation list automatically. |
| **Email** | Required, must not already belong to an account. This is the person's login. |
| **Needs invoice for organization** | Ticking it reveals the billing details field. |
| **Country** | Optional. |
| *(Survey questions)* | The same registration survey the person would have filled in themselves. Required questions must be answered here too. |

The new user always gets the **Author** role; change it afterwards from the user
detail if they should be a Reviewer or Editor.

They receive one e-mail — **Account Created by Organizer** — with a link to set
their own password, valid for 7 days. You never see or set their password. If the
link expires, they can use **Forgot password** on the login screen. Edit the
wording on **[Email templates](/settings/email-templates/)**.

If the e-mail cannot be sent (mail server down, or the **Account Created by
Organizer** template disabled), the account is still created and you get a
warning instead of a confirmation. Fix the cause, then use **Resend set-password
link** in the **⋯** menu on the user detail.

<Aside type="tip" title="Add User or Invitation?">
**Add User** is for people whose details you already have — you type them in and
the account exists. An **[Invitation](/managing/invitations/)** sends a
registration link instead, letting the person fill in their own details, and is
the only way to hand out a Reviewer, Editor, or Admin role up front.
</Aside>
...
| **Resend set-password link** | Sends the **Account Created by Organizer** e-mail again with a fresh link (valid 7 days). Use it when the first one never arrived, expired, or was lost. |
| **Add submission** | Open the submission form to enter a submission **on this user's behalf** (same as the button on the Submissions card — see [below](#add-a-submission-on-this-users-behalf)). |
...
| **Fee Status** | A green **Fee Paid** callout (type, amount, currency, and paid-on date, with **Unmark**) or a red **Fee Unpaid** callout. When unpaid, **Mark as Paid** records payment (fee type / currency from **[Fee](/settings/fee/)**). The **Fee** badge on the [users list](#the-list) does the same thing in one click. |
| **Survey Responses** | The user's answers to your registration **[Survey](/settings/survey/)**. Use **Edit** (top-right of the card) to correct or fill in answers on the user's behalf — see [below](#edit-survey-answers). |
...
![On-behalf submission form opened from a user's detail](../../../assets/screenshots/56-managing-user-add-submission.png)
...
- An [AI assistant](/managing/ai-assistant/) can do the same thing. For a
  file-based type it creates the draft and uploads the file separately, because
  the file cannot travel through the assistant.
...
**Edit** on the Survey Responses card opens a dialog to change or fill in this
user's survey answers on their behalf. It lists **all active survey questions**
(not only the ones already answered), pre-filled with the current answers, so you
can also complete questions the user left blank.

![Edit survey answers dialog on a user's detail](../../../assets/screenshots/57-managing-user-edit-survey-answers.png)

- Every question type works as on the registration form (text, checkbox, single-
  and multi-select, plus the "Other" option where enabled). Required questions are
  validated before saving.
- Saved answers replace the user's own — they appear immediately in the Survey
  Responses card and in the users-list survey columns / XLSX export.
- Available to **Editors and Admins**.
